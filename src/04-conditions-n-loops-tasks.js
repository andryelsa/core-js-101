...
function findFirstSingleChar(str) {
  ...
  const result = newStr.filter((item) => str.lastIndexOf(item) === str.indexOf(item));
  if (result.length === 0) {
    return null;
  }
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (open.includes(currentBracket)) {
      if (close.includes(currentBracket) && stack[stack.length - 1] === currentBracket) {
        stack.pop();
      } else stack.push(currentBracket);
    } else if (close.includes(currentBracket)) {
      const openBracket = stack.pop();
      if (close.indexOf(currentBracket) !== open.indexOf(openBracket)) {
        return false;
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstPath.length; i += 1) {
    ...
    if (pathSegments.every((path) => path[i] === currentSegment)) {
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    if (Eq(position[0][i], position[1][i], position[2][i])) {
      return position[0][i];
    }
    if (Eq(position[i][0], position[i][1], position[i][2])) {
      return position[i][0];
    }
  }

  if (Eq(position[0][0], position[1][1], position[2][2])) {
    return position[0][0];
  }
  if (Eq(position[0][2], position[1][1], position[2][0])) {
    return position[0][2];
  }
  return undefined;
}
